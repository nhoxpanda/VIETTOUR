...
namespace CRMVIETTOUR.Utilities
{
    ...
    public static class LoadData
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static List<TagsViewModel> TinhThanh()
        {
            var model = CacheLayer.Get<List<TagsViewModel>>("TinhThanh");
            if (model == null)
            {
                ...
                CacheLayer.Add<List<TagsViewModel>>(model, "TinhThanh", 1);
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static List<TagsViewModel> QuanHuyen(int? id)
        {
            var model = CacheLayer.Get<List<TagsViewModel>>("QuanHuyen" + id);
            if (model == null)
            {
                model = _db.tbl_Tags.Where(p => p.IsDelete == false && p.TypeTag == 6 && p.ParentId == id)
                            .Select(p => new TagsViewModel
                            {
                                Id = p.Id,
                                Tags = p.Tag
                            }).OrderBy(p => p.Tags).ToList();
                CacheLayer.Add<List<TagsViewModel>>(model, "QuanHuyen" + id, 1);
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static List<TagsViewModel> PhuongXa(int? id)
        {
            var model = CacheLayer.Get<List<TagsViewModel>>("PhuongXa" + id);
            if (model == null)
            {
                model = _db.tbl_Tags.Where(p => p.IsDelete == false && p.TypeTag == 7 && p.ParentId == id)
                            .Select(p => new TagsViewModel
                            {
                                Id = p.Id,
                                Tags = p.Tag
                            }).OrderBy(p => p.Tags).ToList();
                CacheLayer.Add<List<TagsViewModel>>(model, "PhuongXa" + id, 1);
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static List<tbl_Customer> CustomerTourList(int id)
        {
            var model = CacheLayer.Get<List<tbl_Customer>>("CustomerTourList" + id);
            if (model == null)
            {
                model = _db.tbl_TourCustomer.Where(p => p.TourId == id && p.IsDelete == false)
                    .Select(p => p.tbl_Customer).ToList();
                CacheLayer.Add<List<tbl_Customer>>(model, "CustomerTourList" + id, 1);
            }
        ...
        public static List<tbl_Customer> CompanyList()
        {
            ...
            if (model == null)
            {
                model = _db.tbl_Customer.AsEnumerable()
                    .Where(p => p.IsDelete == false && p.CustomerType == CustomerType.Organization
                            && (p.StaffId == clsPermission.GetUser().StaffID || p.StaffManager == clsPermission.GetUser().StaffID)
                            || p.tbl_Tour.Where(x => (x.Permission != null && x.Permission.Contains(clsPermission.GetUser().StaffID.ToString()))
                            || x.StaffId == clsPermission.GetUser().StaffID || x.CreateStaffId == clsPermission.GetUser().StaffID).ToList().Count() > 0)
                    .Select(p => new tbl_Customer
        ...
        public static List<tbl_Customer> PersonalList()
        {
            ...
            {
                model = _db.tbl_Customer.AsEnumerable()
                    .Where(p => p.IsDelete == false && p.CustomerType == CustomerType.Personal
                        && (p.StaffId == clsPermission.GetUser().StaffID || p.StaffManager == clsPermission.GetUser().StaffID)
                        || p.tbl_Tour.Where(x => (x.Permission != null && x.Permission.Contains(clsPermission.GetUser().StaffID.ToString()))
                        || x.StaffId == clsPermission.GetUser().StaffID || x.CreateStaffId == clsPermission.GetUser().StaffID).ToList().Count() > 0)
                    .Select(p => new tbl_Customer
        ...
        /// <summary>
        /// danh sách nhóm cơ hội
        /// </summary>
        /// <returns></returns>
        public static List<DictionaryViewModel> OpportunityGroupList()
        {
            var model = CacheLayer.Get<List<DictionaryViewModel>>("OpportunityGroupList");
            if (model == null)
            {
                model = _db.tbl_Dictionary.Where(p => p.DictionaryCategoryId == 32).Where(p => p.IsDelete == false).Select(p => new DictionaryViewModel
                {
                    Id = p.Id,
                    Name = p.Name
                }).ToList();
                CacheLayer.Add<List<DictionaryViewModel>>(model, "OpportunityGroupList", 1);
            }
        ...
        /// <summary>
        /// danh sách hình thức tiếp xúc
        /// </summary>
        /// <returns></returns>
        public static List<DictionaryViewModel> ContactFormList()
        {
            var model = CacheLayer.Get<List<DictionaryViewModel>>("ContactFormList");
            if (model == null)
            {
                model = _db.tbl_Dictionary.Where(p => p.DictionaryCategoryId == 31).Where(p => p.IsDelete == false).Select(p => new DictionaryViewModel
                {
                    Id = p.Id,
                    Name = p.Name
                }).ToList();
                CacheLayer.Add<List<DictionaryViewModel>>(model, "ContactFormList", 1);
            }
        ...
        /// <summary>
        /// danh sách tình trạng cơ hội
        /// </summary>
        /// <returns></returns>
        public static List<DictionaryViewModel> OpportunityStatusList()
        {
            var model = CacheLayer.Get<List<DictionaryViewModel>>("OpportunityStatusList");
            if (model == null)
            {
                model = _db.tbl_Dictionary.Where(p => p.DictionaryCategoryId == 30).Where(p => p.IsDelete == false).Select(p => new DictionaryViewModel
                {
                    Id = p.Id,
                    Name = p.Name
                }).ToList();
                CacheLayer.Add<List<DictionaryViewModel>>(model, "OpportunityStatusList", 1);
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static List<DictionaryViewModel> TransactionTypeList()
        {
            var model = CacheLayer.Get<List<DictionaryViewModel>>("TransactionTypeList");
            if (model == null)
            {
                model = _db.tbl_Dictionary.Where(p => p.DictionaryCategoryId == 33).Where(p => p.IsDelete == false).Select(p => new DictionaryViewModel
                {
                    Id = p.Id,
                    Name = p.Name
                }).ToList();
                CacheLayer.Add<List<DictionaryViewModel>>(model, "TransactionTypeList", 1);
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static List<tbl_Module> ModuleList()
        {
            var model = CacheLayer.Get<List<tbl_Module>>("ModuleList");
            if (model == null)
            {
                model = _db.tbl_Module.Where(p => p.IsHide == false).OrderBy(p => p.Orders).ToList();
                CacheLayer.Add<List<tbl_Module>>(model, "ModuleList", 1);
            }
            return model;
        }

        /// <summary>
        /// danh sách form theo module
        /// </summary>
        /// <returns></returns>
        public static List<tbl_Form> FormByModuleList(int id)
        {
            var model = _db.tbl_Form.Where(p => p.ModuleId == id && p.IsMenu == true).ToList();
            return model;
        }
    }
}