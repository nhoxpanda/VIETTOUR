...
namespace CRMVIETTOUR.Controllers.Visa
{
    public class ListVisaManageController : BaseController
    {
        ...
        [HttpPost]
        public ActionResult FilterCustomerStaff(int id, int idtag, int type, int status, DateTime? startDate, DateTime? endDate)
        {
            Session["ListVisa"] = null;
            Permission(clsPermission.GetUser().PermissionID, 1101);
            var model = new List<VisaListViewModel>();
        ...
        public ActionResult ExcelSample()
        {
            try
            {
                ...
                using (var stream = new MemoryStream())
                {
                    string templateFile = System.Web.HttpContext.Current.Server.MapPath("~\\Upload\\ImportExport\\Import_VisaCRMVIETTOUR.xlsx");
                    ExportExcelTemplateVisa(stream, templateFile, header);
                    bytes = stream.ToArray();
                }

                string fileName = "Mau-import-visa-CRMVIETTOUR.xlsx";
        ...
        public async Task<ActionResult> CreateCustomer(CustomerViewModel model, FormCollection form)
        {
            try
            {
                ...
                model.SinglePersonal.TagsId = model.SinglePersonal.WardId + "," + model.SinglePersonal.DistrictId + "," + model.SinglePersonal.ProvinceId;
                ...
                {
                    UpdateHistory.SaveHistory(1101, "Thêm mới khách hàng cá nhân, code: " + model.SinglePersonal.Code + " - " + model.SinglePersonal.FullName);
                }
            }
            catch
            {
            ...
            var list = _customerRepository.GetAllAsQueryable().Where(p => p.IsDelete == false).OrderByDescending(p => p.CreatedDate).ToList();
            return PartialView("_Partial_ListNewCustomer", list);
        }
        #endregion

        #region Check ngày hết hạn

        public ActionResult CheckVisa()
        {
            ...
            var checkCustomer = _customerVisaRepository.GetAllAsQueryable().AsEnumerable()
                ...
                && DateTime.Now.AddDays(Convert.ToDouble(info.FirstOrDefault(i => i.TagId == p.TagsId).ExpirationDay)).ToString("dd/MM/yyyy")
                == p.ExpiredDateVisa.Value.ToString("dd/MM/yyyy")).OrderBy(p => p.ExpiredDateVisa).ToList();
            string date = DateTime.Now.AddDays(30).ToString("dd/MM/yyyy");
            return PartialView("_Partial_NotificationVisa", checkCustomer);
        }
