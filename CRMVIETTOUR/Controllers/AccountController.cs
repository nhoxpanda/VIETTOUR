...
namespace CRMVIETTOUR.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(string returnUrl)
        {
            ...
            //UserManager.RemovePassword("f5d74141-6aaa-4442-b4f9-4886dfe971dd");
            //UserManager.AddPassword("f5d74141-6aaa-4442-b4f9-4886dfe971dd", "Admin@123");
            return View();
        }
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (user != null)
                    {
                        ...
                        {
                            var item = _db.tbl_Staff.FirstOrDefault(p => p.Code == model.UserName && p.IsDelete == false);
                            //string mk = Common.MaHoa(item.Password);
                            if (item != null)
                            {
        ...
        #region Password
        [HttpPost]
        public async Task<ActionResult> CreatePassword(int id, string pass)
        {
            var staff = _db.tbl_Staff.Find(id);
            if (UserManager.FindByName(staff.Code) != null) // đã có account
            {
                // change password = code
                UserManager.RemovePassword(UserManager.FindByName(staff.Code).Id);
                UserManager.AddPassword(UserManager.FindByName(staff.Code).Id, pass);
            }
            ...
            staff.Password = Common.MaHoa(pass);
            _db.SaveChanges();
