...
namespace CRMVIETTOUR.Controllers.Other
{
    ...
    public class NationManageController : BaseController
    {
        ...
        public NationManageController(IGenericRepository<tbl_Dictionary> dictionaryRepository, IBaseRepository baseRepository)
            : base(baseRepository)
        {
        ...
        public async Task<ActionResult> Delete(FormCollection fc)
        {
            if (fc["listItemId"] != null && fc["listItemId"] != "")
            {
                ...
                if (listIds.Count() > 0)
                {
                    ...
                    {
                        return Json(new ActionModel() { Succeed = true, Code = "200", View = "", Message = "Xóa dữ liệu thành công !", IsPartialView = false, RedirectTo = Url.Action("Index", "NationManage") }, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
