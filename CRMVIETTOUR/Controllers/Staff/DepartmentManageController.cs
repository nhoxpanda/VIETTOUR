...
namespace CRMVIETTOUR.Controllers.Staff
{
    ...
    public class DepartmentManageController : BaseController
    {
        ...
        public DepartmentManageController(IGenericRepository<tbl_Dictionary> dictionaryRepository, IBaseRepository baseRepository)
            : base(baseRepository)
        {
        ...
        public ActionResult Index()
        {
            ...
            var dictionary = _dictionaryRepository.GetAllAsQueryable().Where(p => p.DictionaryCategoryId == 6 && p.IsDelete == false)
        ...
        public async Task<ActionResult> Delete(FormCollection fc)
        {
            if (fc["listItemId"] != null && fc["listItemId"] != "")
            {
                ...
                if (listIds.Count() > 0)
                {
                    ...
                    {
                        return Json(new ActionModel() { Succeed = true, Code = "200", View = "", Message = "Xóa dữ liệu thành công !", IsPartialView = false, RedirectTo = Url.Action("Index", "DepartmentManage") }, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
