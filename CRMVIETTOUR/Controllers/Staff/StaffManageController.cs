    ...
    public class StaffManageController : BaseController
    {
        ...
        public ActionResult ExportFile()
        {
            ...
            try
            {
                ...
                return File(bytes, "text/xls", "Mau-import-nhan-vien.xlsx");
            }
            catch (Exception)
            {
            }
        ...
        public virtual void ExportCustomersToXlsx(Stream stream, IList<StaffExportNew> staffs)
        {
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                worksheet.Cells["A3:W4"].Value = "DANH SÁCH NHÂN VIÊN";
        ...
        public ActionResult ExcelSample()
        {
            try
            {
                ...
                using (var stream = new MemoryStream())
                {
                    string templateFile = System.Web.HttpContext.Current.Server.MapPath("~\\Upload\\ImportExport\\Import_NhanVienCRMVIETTOUR.xlsx");
                    ExportExcelTemplateCustomer(stream, templateFile, header);
                    bytes = stream.ToArray();
                }

                string fileName = "Mau-import-nhan-vien-CRMVIETTOUR.xlsx";
