...
namespace CRMVIETTOUR.Controllers.Tour
{
    ...
    public class TourScheduleController : BaseController
    {
        ...
        public ActionResult TourScheduleFilter(int id, DateTime? start, DateTime? end, int leader)
        {
            Permission(clsPermission.GetUser().PermissionID, 72);
            ...
            {
                var model = _tourRepository.GetAllAsQueryable().AsEnumerable().Where(p => p.TypeTourId == id
        ...
        #region JsonCalendar
        [HttpPost]
        public JsonResult JsonCalendar(int id, DateTime? start, DateTime? end, int leader)
        {
            if (id == -1)
            {
                var _model = _tourRepository.GetAllAsQueryable().AsEnumerable()
