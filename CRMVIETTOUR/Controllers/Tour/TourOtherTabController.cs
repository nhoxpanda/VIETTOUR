...
namespace CRMVIETTOUR.Controllers.Tour
{
    ...
    public class TourOtherTabController : BaseController
    {
        ...
        private IGenericRepository<tbl_TourCustomerService> _tourCustomerServiceRepository;
        ...
        public TourOtherTabController(
            ...
            IGenericRepository<tbl_TourCustomerService> tourCustomerServiceRepository,
            IBaseRepository baseRepository)
            : base(baseRepository)
        {
            this._accessDataRepository = accessDataRepository;
            this._tourCustomerServiceRepository = tourCustomerServiceRepository;
        ...
        void Permission(int PermissionsId, int formId)
        {
            ...
            var ltAccess = _accessDataRepository.GetAllAsQueryable().AsEnumerable()
                            .Where(p => p.PermissionId == PermissionsId && p.FormId == formId)
                            .Select(p => p.ShowDataById).FirstOrDefault();
            if (ltAccess != 0)
                this.SDBID = ltAccess;
        ...
        public string LoadGuide(int id)
        {
            var tour = _tourRepository.GetAllAsQueryable().AsEnumerable().FirstOrDefault(p => p.Id == id);
            var tourGuide = _tourGuideRepository.GetAllAsQueryable().AsEnumerable()
                .FirstOrDefault(x => x.TourId == id && x.StartDate != null && tour.StartDate != null
                && x.StartDate.Value.ToShortDateString() == tour.StartDate.Value.ToShortDateString() && x.IsDelete == false);
            if (tourGuide != null)
            {
                if (tourGuide.StaffId != null)
                {
                    return tourGuide.tbl_Staff.FullName;
                }
                return "";
            }
            return "";
        ...
        public async Task<ActionResult> EditLiabilityCustomer(int id)
        {
            ...
            ViewBag.ServiceList = _tourCustomerServiceRepository.GetAllAsQueryable().Where(p => p.CustomerId == model.CustomerId).ToList();
            Session["idTour"] = model.TourId;
        ...
        public async Task<ActionResult> CreateLiabilityCustomer(tbl_LiabilityCustomer model, FormCollection form)
        {
            try
            {
                ...
                if (await _liabilityCustomerRepository.Create(model))
                {
                    ...
                    var list = _liabilityCustomerRepository.GetAllAsQueryable().AsEnumerable()
                        .Where(p => p.TourId == model.TourId && p.IsDelete == false)
                        .Select(p => new LiabilityCustomerViewModel
                        {
                            Customer = p.CustomerId != null ? p.tbl_Customer.FullName : "",
                            Sale = p.CustomerId != null ? p.tbl_Customer.tbl_StaffManager.FullName : "",
                            Guide = LoadGuide(p.TourId),
                            FullNote = p.FullNote,
                            Price1 = p.FirstPrice != null ? string.Format("{0:0,0}", p.FirstPrice).Replace(',', '.') : "",
                            Date1 = p.FirstDate != null ? string.Format("{0:dd-MM-yyyy}", p.FirstDate) : "",
                            Price2 = p.SecondPrice != null ? string.Format("{0:0,0}", p.SecondPrice).Replace(',', '.') : "",
                            Date2 = p.SecondDate != null ? string.Format("{0:dd-MM-yyyy}", p.SecondDate) : "",
                            Note = p.Note,
                            Id = p.Id,
                            TotalContract = p.TotalContract != null ? string.Format("{0:0,0}", p.TotalContract).Replace(',', '.') : "",
                            TotalReturn = p.TotalReturn != null ? string.Format("{0:0,0}", p.TotalReturn).Replace(',', '.') : "",
                            TotalVisa = p.TotalVisa != null ? string.Format("{0:0,0}", p.TotalVisa).Replace(',', '.') : "",
                            IsBill = p.IsBill == true ? "Có lấy HĐ" : "Không lấy HĐ",
                            IsContract = p.IsContract == true ? "Rồi" : "",
                            IsPPVS = p.IsPPVS == true ? "Rồi" : "",
                            TourId = p.TourId,
        ...
        public async Task<ActionResult> UpdateLiabilityCustomer(tbl_LiabilityCustomer model, FormCollection form)
        {
            try
            {
                ...
                model.TotalContract = !string.IsNullOrEmpty(form["TotalContract"]) ? Convert.ToDecimal(form["TotalContract"].ToString()) : 0;
                model.FirstPrice = !string.IsNullOrEmpty(form["FirstPrice"]) ? Convert.ToDecimal(form["FirstPrice"].ToString()) : 0;
                model.SecondPrice = !string.IsNullOrEmpty(form["SecondPrice"]) ? Convert.ToDecimal(form["SecondPrice"].ToString()) : 0;
                model.TotalContract = !string.IsNullOrEmpty(form["TotalContract"]) ? Convert.ToDecimal(form["TotalContract"].ToString()) : 0;
                model.TotalReturn = !string.IsNullOrEmpty(form["TotalReturn"]) ? Convert.ToDecimal(form["TotalReturn"].ToString()) : 0;
                model.TotalVisa = !string.IsNullOrEmpty(form["TotalVisa"]) ? Convert.ToDecimal(form["TotalVisa"].ToString()) : 0;
                ...
                if (await _liabilityCustomerRepository.Update(model))
                {
                    ...
                    // lưu danh sách dịch vụ khách hàng
                    var listS = _tourCustomerServiceRepository.GetAllAsQueryable().AsEnumerable()
                            .Where(p => p.CustomerId == model.CustomerId).ToList();
                    foreach (var item in listS)
                    {
                        await _tourCustomerServiceRepository.DeleteMany(item.Id.ToString().Split(',').ToArray(), true);
                    }
                    for (int i = 1; i <= Convert.ToInt32(form["CountService"]); i++)
                    {
                        var service = new tbl_TourCustomerService()
                        {
                            CreatedDate = DateTime.Now,
                            CustomerId = model.CustomerId ?? 0,
                            Quantity = !string.IsNullOrEmpty(form["txtQuantityS" + i]) ? Convert.ToInt32(form["txtQuantityS" + i].Replace(",", "").Replace(".", "")) : 0,
                            UnitPrice = !string.IsNullOrEmpty(form["txtUnitPriceS" + i]) ? Convert.ToDouble(form["txtUnitPriceS" + i].Replace(",", "").Replace(".", "")) : 0,
                            TotalPrice = !string.IsNullOrEmpty(form["txtTotalPriceS" + i]) ? Convert.ToDouble(form["txtTotalPriceS" + i].Replace(",", "").Replace(".", "")) : 0,
                            ServiceName = form["txtNameS" + i],
                            CurrencyId = 1209,
                            LiabilityCustomerId = model.Id,
                        };
                        totalprice += (service.UnitPrice * service.Quantity);
                        await _tourCustomerServiceRepository.Create(service);
                    }
                    // cập nhật tổng tiền
                    var price = _db.tbl_LiabilityCustomer.Find(model.Id);
                    price.TotalLiquidation = (decimal?)totalprice;
                    _db.SaveChanges();
                    //
                    var list = _liabilityCustomerRepository.GetAllAsQueryable().AsEnumerable()
                        .Where(p => p.TourId == model.TourId && p.IsDelete == false)
        ...
        public async Task<ActionResult> DeleteLiabilityCustomer(int id)
        {
            try
            {
                ...
                {
                    var list = _liabilityCustomerRepository.GetAllAsQueryable().AsEnumerable()
                        .Where(p => p.TourId == tourId && p.IsDelete == false)
                        .Select(p => new LiabilityCustomerViewModel
                        {
                            Customer = p.CustomerId != null ? p.tbl_Customer.FullName : "",
                            Sale = p.CustomerId != null ? p.tbl_Customer.tbl_StaffManager.FullName : "",
        ...
        public async Task<ActionResult> DeleteService(int id)
        {
            ...
            {
                Permission(clsPermission.GetUser().PermissionID, 84);
                var tourService = _tourCustomerServiceRepository.FindId(id);
                await _tourCustomerServiceRepository.DeleteMany(id.ToString().Split(',').ToArray(), true);
                // cập nhật tổng tiền
                var price = _db.tbl_LiabilityCustomer.Find(tourService.LiabilityCustomerId);
                price.TotalLiquidation -= (decimal?)tourService.TotalPrice;
                _db.SaveChanges();
                return Json(JsonRequestBehavior.AllowGet);
            }
            catch
        ...
        public async Task<ActionResult> DeleteVisa(int id)
        {
            ...
            try
            {
                ...
                //
                //var itemcv = _customerVisaRepository.FindId(id).tbl_TourCustomerVisa;
                    ...
                    if (listIds.Count() > 0)
                    {
                        foreach (var id in listIds)
                        {
                            var check = _tourCustomerRepository.GetAllAsQueryable().AsEnumerable()
                                .FirstOrDefault(p => p.TourId == idTour && p.CustomerId.ToString() == id);
                            if (check == null)
                            {
                                // lưu khách hàng vào tour
                                var cit = new tbl_TourCustomer
                                {
                                    CustomerId = Convert.ToInt32(id),
                                    IsDelete = false,
                                    TourId = idTour
                                };
                                await _tourCustomerRepository.Create(cit);
                                // lưu vào công nợ khách hàng
                                var cn = new tbl_LiabilityCustomer()
                                {
                                    CreateDate = DateTime.Now,
                                    CustomerId = Convert.ToInt32(id),
                                    IsDelete = false,
                                ...
                                await _liabilityCustomerRepository.Create(cn);
                            }
                        }
                    }
                }
                var model = _db.tbl_TourCustomer.AsEnumerable().Where(c => c.TourId == idTour && c.IsDelete == false)
                    .Select(c => new tbl_Customer
                    {
                        Id = c.tbl_Customer.Id,
                        IsTemp = c.tbl_Customer.IsTemp,
                        Code = c.tbl_Customer.Code,
                        FullName = c.tbl_Customer.FullName,
                        Address = c.tbl_Customer.Address,
                        Phone = c.tbl_Customer.Phone,
                        IdentityCard = c.tbl_Customer.IdentityCard,
                        PassportCard = c.tbl_Customer.PassportCard,
                        CreatedDatePassport = c.tbl_Customer.CreatedDatePassport,
                        ExpiredDatePassport = c.tbl_Customer.ExpiredDatePassport,
                        NoteTour = c.tbl_Customer.NoteTour
                    }).ToList();
                return PartialView("~/Views/TourTabInfo/_KhachHang.cshtml", model);
            }
            catch
            {
                return PartialView("~/Views/TourTabInfo/_KhachHang.cshtml", new List<tbl_Customer>());
            }
        }
        ...
        {
            int idTour = Int32.Parse(Session["idTour"].ToString());
            var model = _tourCustomerRepository.GetAllAsQueryable().FirstOrDefault(p => p.TourId == idTour && p.CustomerId == id);
            if (model != null)
            {
                await _tourCustomerRepository.DeleteMany(model.Id.ToString().Split(',').ToArray(), false);
                return Json(1, JsonRequestBehavior.AllowGet);
            }
