function addRow2Colum() {
    $("table.table-addrow tbody").append("<tr><td><input type='checkbox' value='0' id='cb' name='cb' checked='checked' /></td>"
        ...
        + "<td><input type='number' value='' id='txtNumber' class='form-control' /></td>"
...
function updateRow2Colum() {
    $('tbody tr td').find("input:checkbox").each(function () {
        if (this.checked) {
            ...
            var text1 = tr1.text();
            tr1.text("");
            tr1.append("<input type='number' id='txtNumber' value='" + text1 + "' class='form-control' />");
            //
            var tr2 = tr1.prev();
            var text2 = tr2.text();
            tr2.text("");
            tr2.append("<input type='text' id='txtName' value='" + text2 + "' class='form-control' />");
        }
    });
    $("#btnAdd").attr("disabled", "disabled");
...
$("#btnSave").click(function () {
    ...
    $.ajax({
        type: "POST",
        url: '/OpportunityStatusManage/SaveData',
        data: JSON.stringify(dataPost),
        contentType: "application/json; charset=utf-8",
        dataType: "html",
        success: function (data) {
            window.location.href = '/OpportunityStatusManage/Index';
        }
    });
})
